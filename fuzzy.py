...
def fuzzyy():
    fuzzy_data=clean_data()
    fuzzy_data=fuzzy_data['Place of interest'].to_list().np.unique()
    ins_deportiva=input('Introduce una Instalación Deportiva: ')
    solution=process.extractOne(ins_deportiva,fuzzy_data, score_cutoff=90)
    if solution is None:
        solution = 'None Sport Centre matching'
    return solution[0]
